...
$themeData['view_title'] = Lang('privacy_label');
...
if (@$seller['system'] || @$seller['moderator']) {
    ...
    if (!empty($inputLang)) {
        foreach ($inputLang as $key => $value) {
            $fetch = $utiObj->getMiscInfo('privacy_policy', $value['langcode']);
            $ItemData = (!empty($fetch)) ? $fetch : array('title' => '', 'content' => '');
            if (count($inputLang) == 1) {
                $themeData['lang_name'] = Lang('privacy_label');
            } else {
    ...
    $themeData['footer_scripts_p'] .= ' 
		<script src="' . $config['theme_url'] . '/backend/jquery/misc/privacy.js" type="text/javascript"></script>';
}
