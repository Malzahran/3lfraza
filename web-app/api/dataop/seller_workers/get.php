...
if (@$seller['system'] || @$seller['moderator']) {
    $sAdmin = true;
    $allowbuttons = 1;
    $intentdata = array('intent' => 6, 'reqtype' => 'seller_workers', 'reqstype' => 'add_form', 'reqptype' => 'add');
    $buttonst[] = array(
        'id' => $btnid += 1,
        'place' => 2,
        'color' => 9,
...
if (!empty($fetch['data'])) {
    ...
    foreach ($fetch['data'] as $v) {
        ...
        if ($sAdmin) {
            $intentdata = array('intent' => 2, 'reqtype' => 'seller_workers', 'reqstype' => 'view_info', 'reqid' => $id);
            $buttons[] = array(
                'id' => $btnid += 1,
                'text' => Lang('view_info_btn'),
                'intentData' => $intentdata
            );
            $intentdata = array('intent' => 6, 'reqtype' => 'seller_workers', 'reqstype' => 'edit_form', 'reqptype' => 'edit', 'reqid' => $id);
            $buttons[] = array(
                'id' => $btnid += 1,
                'color' => 2,
                'text' => Lang('edit_btn'),
            ...
            $intentdata = array('intent' => 6, 'reqtype' => 'seller_workers', 'reqstype' => 'delete_confirm', 'reqptype' => 'delete', 'reqid' => $id);
            $buttons[] = array(
                'id' => $btnid += 1,
                'color' => 1,
        ...
        $items[] = array(
            ...
            'contact_title' => Lang('contact_worker_label'),
